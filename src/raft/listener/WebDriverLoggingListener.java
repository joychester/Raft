...
public class WebDriverLoggingListener extends AbstractWebDriverEventListener {
	...
	public void beforeNavigateTo(String url, WebDriver driver) {
		logger.println(logDateFormat.format(new Date()) +	": navigate(url): " + url);
		startTime = System.currentTimeMillis(); //update start time
    }
	public void afterNavigateTo(String url, WebDriver driver) {
		logger.print(logDateFormat.format(new Date()) +	": after navigate(url): " + driver.getCurrentUrl());
		logDuration();
    }
	
	public void beforeClickOn(WebElement element, WebDriver driver) {
		logger.println(logDateFormat.format(new Date()) +	": click(element text): " + element.getText());
		startTime = System.currentTimeMillis(); //update start time
    }
	public void afterClickOn(WebElement element, WebDriver driver) {
		logger.print(logDateFormat.format(new Date()) +	": after click(current url): " + driver.getCurrentUrl());
		logDuration();
    }
	
	//clear(), sendKeys(), toggle()
	public void beforeChangeValueOf(WebElement element, WebDriver driver) {
		logger.println(logDateFormat.format(new Date()) +	": value(before): " + element.getValue());
    }
    public void afterChangeValueOf(WebElement element, WebDriver driver) {
    	logger.println(logDateFormat.format(new Date()) +	": value(after): " + element.getValue());
    }
    ...
    public void beforeScript(String script, WebDriver driver) {
    	logger.println(logDateFormat.format(new Date()) +	": call javascript: " + script);
		startTime = System.currentTimeMillis(); //update start time
    }
    public void afterScript(String script, WebDriver driver) {
    	logger.print(logDateFormat.format(new Date()) +	": after call javascript");
		logDuration();
    }
	...
	private void logDuration() {
		...
		String respStr = " duration: " + duration + "ms";
		if( duration < 6000 ) ;
